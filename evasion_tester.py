...
print("="*65)
print("   CYBER-SENTINEL: ADVERSARIAL FUZZER (FULL SPECTRUM)   ")
print("="*65)
...
print("[?] Select Attack Intensity (Applies only to Bot/Infiltration):")
print(" [1] Subtle (Small changes)")
print(" [2] Realistic (Standard network variations)")
print(" [3] Stress Test (Drastic changes)")
...
try:
    MAX_ATTEMPTS = int(input("[?] How many total samples would you like to test? "))
except ValueError:
    MAX_ATTEMPTS = 1000
...
classes_to_test = ['Benign', 'Infiltration', 'Bot']
features_to_test = [
...
stats = {
    'Benign': {'attempts': 0, 'correct': 0, 'fp': 0, 'failed': 0},
...
for i in range(1, MAX_ATTEMPTS + 1):
    ...
    # _____5. LOGIC SPLIT_____
    if current_class == 'Benign':
        # No Multiplier for Benign
        res = query_api(modified)
        if res == "Benign":
            stats['Benign']['correct'] += 1
        else:
            stats['Benign']['fp'] += 1
    
    else:
        # Attack Logic (Infiltration/Bot) gets the Multiplier
    ...
    if i % step == 0:
        ...
        print(f"--- PROGRESS: {i}/{MAX_ATTEMPTS} ({percent:.0f}%) | Bypasses so far: {found_total} ---")

# _____6. FINAL SUMMARY REPORT_____
...
b = stats['Benign']
if b['attempts'] > 0:
    fp_rate = (b['fp'] / b['attempts'] * 100)
    print(f"\nCLASS: Benign (Control Group)")
    print(f"  > Samples Tested: {b['attempts']}")
    print(f"  > Correctly Blocked/Identified: {b['correct']}")
    print(f"  > False Positives (Incorrectly flagged as attack): {b['fp']} (Rate: {fp_rate:.2f}%)")
